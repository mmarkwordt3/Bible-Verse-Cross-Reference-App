...
export function validateReviewData(data,options={}){
  ...
  if(data.schemaVersion!==1)at('schemaVersion must be 1');
  ...
  for(const [index,review] of data.reviews.entries()){
    ...
    if(!RELATIONSHIP_TYPES.includes(review?.relationshipType))at('invalid relationshipType',id);
    if(!Number.isInteger(review?.reviewerConfidence)||review.reviewerConfidence<1||review.reviewerConfidence>5)at('reviewerConfidence must be an integer from 1 through 5',id);
    if(!CHRONOLOGY_CONFIDENCES.includes(review?.chronologyConfidence))at('invalid chronologyConfidence',id);
    if(!sized(review?.independenceGroup,120))at('independenceGroup must be a string of at most 120 characters',id);
    if(!sized(review?.notes,10000))at('notes must be a string of at most 10,000 characters',id);
